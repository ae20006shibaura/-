...
from apps import models as modelpict
@pictapp.route('/user-list/,int:user_id>')
@login_required
def user_list(user_id):
    stmt=select(
        modelpict.UserPicture).filter_by(user_id).order_by(modelpict.UserPicture.create_at.desc())
    userlist=db.session.excute(stmt).scalars().all()
